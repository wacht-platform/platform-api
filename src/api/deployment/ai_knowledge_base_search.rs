...
use crate::{
    application::{
        AppError, AppState,
        response::ApiResult,
        http::models::json::ai_knowledge_base::{
            SearchKnowledgeBaseQuery, SearchKnowledgeBaseResponse, KnowledgeBaseSearchResult,
        },
    },
    core::{
        queries::{ai_knowledge_base::GetAiKnowledgeBaseByIdQuery, Query as QueryTrait},
...
pub async fn search_knowledge_base(
    ...
    let results = if let Some(kb_id) = params.knowledge_base_id {
        ...
        QdrantService::search_similar(query_embedding, limit, kb_id)
            .await?
    } else {
        ...
        return Err(AppError::BadRequest("Please specify a knowledge_base_id parameter for search".to_string()).into());
    };
...
pub async fn search_specific_knowledge_base(
    ...
    let results = QdrantService::search_similar(query_embedding, limit, knowledge_base_id)
        .await?;
