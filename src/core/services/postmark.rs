...
use crate::{
    ...
    core::models::{DnsRecord, EmailVerificationRecords},
};
...
impl PostmarkService {
    ...
    pub fn create_domain(&self, domain_name: &str) -> Result<PostmarkDomain, AppError> {
        ...
        if response.status() != 200 {
            let error_text = response.body_mut().read_to_string()
                .unwrap_or_else(|_| "Unknown error".to_string());
            return Err(AppError::External(format!("Postmark API error: {}", error_text)));
        }

        let domain: PostmarkDomain = response.body_mut().read_json()
            .map_err(|e| AppError::External(format!("Failed to parse Postmark response: {}", e)))?;

        tracing::info!("Successfully created Postmark domain: {} (ID: {})", domain.name, domain.id);
        Ok(domain)
    }
    ...
    pub fn verify_dkim(&self, domain_id: i64) -> Result<PostmarkDomain, AppError> {
        let mut response = ureq::put(&format!("{}/domains/{}/verifyDkim", self.base_url, domain_id))
            .header("Accept", "application/json")
            .header("X-Postmark-Account-Token", &self.account_token)
            .send("")
        ...
        if response.status() != 200 {
            let error_text = response.body_mut().read_to_string()
                .unwrap_or_else(|_| "Unknown error".to_string());
            return Err(AppError::External(format!("Postmark DKIM verification error: {}", error_text)));
        }
    ...
    pub fn verify_return_path(&self, domain_id: i64) -> Result<PostmarkDomain, AppError> {
        let mut response = ureq::put(&format!("{}/domains/{}/verifyReturnPath", self.base_url, domain_id))
            .header("Accept", "application/json")
            .header("X-Postmark-Account-Token", &self.account_token)
            .send("")
        ...
        if response.status() != 200 {
            let error_text = response.body_mut().read_to_string()
                .unwrap_or_else(|_| "Unknown error".to_string());
            return Err(AppError::External(format!("Postmark Return-Path verification error: {}", error_text)));
        }
    ...
    pub fn send_email(
        ...
        if response.status() != 200 {
            let error_text = response.body_mut().read_to_string()
                .unwrap_or_else(|_| "Unknown error".to_string());
            return Err(AppError::External(format!("Postmark email sending error: {}", error_text)));
        }

        let email_response: SendEmailResponse = response.body_mut().read_json()
            .map_err(|e| AppError::External(format!("Failed to parse Postmark response: {}", e)))?;

        if email_response.error_code != 0 {
            return Err(AppError::External(format!("Postmark email error: {}", email_response.message)));
        }
    ...
    pub fn delete_domain(&self, domain_id: i64) -> Result<(), AppError> {
        ...
        if response.status() != 200 {
            let error_text = response.body_mut().read_to_string()
                .unwrap_or_else(|_| "Unknown error".to_string());
            return Err(AppError::External(format!("Postmark domain deletion error: {}", error_text)));
        }
    ...
    pub fn generate_email_verification_records(&self, domain: &PostmarkDomain) -> EmailVerificationRecords {
        ...
        if !domain.return_path_domain.is_empty() && !domain.return_path_domain_cname_value.is_empty() {
