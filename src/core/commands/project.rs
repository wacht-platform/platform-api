...
impl Command for CreateProjectWithStagingDeploymentCommand {
    ...
    async fn execute(self, app_state: &AppState) -> Result<Self::Output, AppError> {
        ...
        sqlx::query(&staging_ui_settings_query)
            ...
            .bind(serde_json::to_value(&ui_settings.light_mode_settings)
                .map_err(|e| AppError::Serialization(e.to_string()))?)
            .bind(serde_json::to_value(&ui_settings.dark_mode_settings)
                .map_err(|e| AppError::Serialization(e.to_string()))?)
        ...
        for provider in social_providers.iter() {
            ...
            if (self.auth_methods.contains(&provider.to_string()) || self.auth_methods.contains(&provider_with_oauth))
        ...
        let deployment = Deployment {
            ...
            verification_status: Some(crate::core::models::VerificationStatus::Verified),
...
impl CreateProductionDeploymentCommand {
    ...
    async fn cleanup_deployment_on_failure(&self, app_state: &AppState, deployment_id: i64) -> Result<(), AppError> {
        tracing::warn!("Cleaning up deployment {} due to external service failure", deployment_id);
        let mut tx = app_state.db_pool.begin().await?;
        ...
        tracing::info!("Successfully cleaned up deployment {} and related records", deployment_id);
        Ok(())
    }
    ...
    async fn cleanup_external_resources_on_failure(
        ...
        if cleanup_frontend {
            if let Err(e) = app_state.cloudflare_service.delete_custom_hostname(frontend_hostname) {
                tracing::error!("Failed to cleanup frontend hostname {}: {}", frontend_hostname, e);
            } else {
                tracing::info!("Successfully cleaned up frontend hostname: {}", frontend_hostname);
            }
        }

        if cleanup_backend {
            if let Err(e) = app_state.cloudflare_service.delete_custom_hostname(backend_hostname) {
                tracing::error!("Failed to cleanup backend hostname {}: {}", backend_hostname, e);
            } else {
                tracing::info!("Successfully cleaned up backend hostname: {}", backend_hostname);
            }
        }
...
impl Command for CreateProductionDeploymentCommand {
    ...
    async fn execute(self, app_state: &AppState) -> Result<Self::Output, AppError> {
        ...
        let project = sqlx::query!("SELECT name FROM projects WHERE id = $1 AND deleted_at IS NULL", self.project_id)
            .fetch_optional(&mut *tx)
            .await?
            .ok_or_else(|| AppError::NotFound(format!("Project with id {} not found", self.project_id)))?;
        ...
        let empty_email_verification_records = crate::core::models::EmailVerificationRecords::default();
        ...
        let auth_settings = self.create_auth_settings(deployment_row.id);
        let ui_settings = self.create_ui_settings(deployment_row.id, frontend_host.clone(), project.name.clone());
        let mut b2b_settings = self.create_b2b_settings(deployment_row.id);
        let restrictions = self.create_restrictions(deployment_row.id);
        let email_templates = self.create_email_templates(deployment_row.id);
        let sms_templates = self.create_sms_templates(deployment_row.id);
        ...
        let empty_credentials = serde_json::to_value(crate::core::models::OauthCredentials::default())
            .map_err(|e| AppError::Serialization(e.to_string()))?;

        for provider in social_providers.iter() {
            let provider_with_oauth = format!("{}_oauth", provider);
            if (self.auth_methods.contains(&provider.to_string()) || self.auth_methods.contains(&provider_with_oauth))
                && crate::core::models::SocialConnectionProvider::from_str(*provider).is_ok()
            {
                sqlx::query!(
                    r#"
                    INSERT INTO deployment_social_connections (
        ...
        let email_verification_records = app_state.postmark_service.generate_email_verification_records(&postmark_domain);
        ...
        let frontend_hostname_id = match frontend_hostname_result {
            Ok(custom_hostname) => {
                ...
                tracing::info!("Successfully created frontend custom hostname: {}", frontend_hostname);
                Some(custom_hostname.id)
            }
            Err(e) => {
                tracing::error!("Failed to create frontend custom hostname: {}", e);

                let _ = self.cleanup_deployment_on_failure(app_state, deployment_row.id).await;
        ...
        let backend_hostname_id = match backend_hostname_result {
            Ok(custom_hostname) => {
                ...
                tracing::info!("Successfully created backend custom hostname: {}", backend_hostname);
                Some(custom_hostname.id)
            }
            Err(e) => {
        ...
        tracing::info!("Postmark domain created successfully for: {}", self.custom_domain);
        ...
        Ok(Deployment {
            ...
            mode: DeploymentMode::from(deployment_row.mode),
            mail_from_host: deployment_row.mail_from_host,
            verification_status: Some(crate::core::models::VerificationStatus::Pending),
...
impl Command for VerifyDeploymentDnsRecordsCommand {
    ...
    async fn execute(self, app_state: &AppState) -> Result<Self::Output, AppError> {
        ...
        app_state
            ...
            .verify_domain_records(&mut domain_verification_records, &app_state.cloudflare_service)
            .map_err(|e| {
        ...
        let final_verification_status = match verification_status {
            "verified" => crate::core::models::VerificationStatus::Verified,
            "in_progress" => crate::core::models::VerificationStatus::InProgress,
            _ => crate::core::models::VerificationStatus::Pending,
        };
...
impl DeleteDeploymentCommand {
    ...
    async fn cleanup_external_resources(&self, app_state: &AppState, deployment: &Deployment) -> Result<(), AppError> {
        tracing::info!("Cleaning up external resources for deployment {}", self.deployment_id);
        ...
        if deployment.mode == DeploymentMode::Production {
            if let Some(domain_records) = &deployment.domain_verification_records {
                if let Some(frontend_hostname_id) = &domain_records.frontend_hostname_id {
                    if let Err(e) = app_state.cloudflare_service.delete_custom_hostname(frontend_hostname_id) {
                        tracing::warn!("Failed to cleanup frontend hostname {}: {}", frontend_hostname_id, e);
                    } else {
                        tracing::info!("Successfully cleaned up frontend hostname: {}", frontend_hostname_id);
                    }
                }

                if let Some(backend_hostname_id) = &domain_records.backend_hostname_id {
                    if let Err(e) = app_state.cloudflare_service.delete_custom_hostname(backend_hostname_id) {
                        tracing::warn!("Failed to cleanup backend hostname {}: {}", backend_hostname_id, e);
                    } else {
                        tracing::info!("Successfully cleaned up backend hostname: {}", backend_hostname_id);
                    }
                }
            }
            ...
            if let Some(email_records) = &deployment.email_verification_records {
                if let Some(postmark_domain_id) = email_records.postmark_domain_id {
                    if let Err(e) = app_state.postmark_service.delete_domain(postmark_domain_id) {
                        tracing::warn!("Failed to cleanup Postmark domain {}: {}", postmark_domain_id, e);
                    } else {
                        tracing::info!("Successfully cleaned up Postmark domain: {}", postmark_domain_id);
                    }
                }
            }
        }
    ...
    async fn cleanup_database_records(&self, app_state: &AppState) -> Result<(), AppError> {
        tracing::info!("Soft deleting database records for deployment {}", self.deployment_id);
        let mut tx = app_state.db_pool.begin().await?;

        let now = chrono::Utc::now();
        tracing::info!("Skipping soft delete of deployment settings tables - only marking deployment as deleted");
        sqlx::query!(
            "UPDATE deployments SET deleted_at = $1, updated_at = $1 WHERE id = $2",
            now,
            self.deployment_id
        ...
        tracing::info!("Successfully cleaned up all database records for deployment {}", self.deployment_id);
        Ok(())
    }
}
...
impl Command for DeleteDeploymentCommand {
    ...
    async fn execute(self, app_state: &AppState) -> Result<Self::Output, AppError> {
        ...
        if deployment_count.count.unwrap_or(0) <= 1 {
            return Err(AppError::BadRequest(
                "Cannot delete the last deployment in a project. Delete the project instead.".to_string()
            ));
        }
        ...
        let deployment_model = Deployment {
            ...
            domain_verification_records: deployment_row.domain_verification_records
                .and_then(|data| serde_json::from_value(data).ok()),
            email_verification_records: deployment_row.email_verification_records
                .and_then(|data| serde_json::from_value(data).ok()),
        };

        if let Err(e) = self.cleanup_external_resources(app_state, &deployment_model).await {
        ...
        tracing::info!("Successfully soft deleted deployment {}", self.deployment_id);
        Ok(())
    }
}
